...
def get_user_id():
    ...
    try:
        return api_request(add_params)["response"][0]['id']
    except KeyError:
...
def get_groups_list(user_id):
    ...
    try: 
        return api_request(add_params)["response"]['items']
    except KeyError:
        print('Error, user account is closed')
...
def is_member(counting_friends, group_id, friends_list, friends_limit=0):
    ...
    try:
        counting_friends = list(map(lambda x: x['member'], api_request(add_params)["response"]))
        return counting_friends
    except KeyError:
        print(f"Group ID: {group_id} - error, this group is not available!")
        return counting_friends

def match_groups_friends(group_id, friends_limit, user_id):            
    counting_friends = []
    offset = 0
    friends_list = get_friends_list(user_id, offset)
    while len(friends_list) != 0:
        counting_friends += (is_member(counting_friends, group_id, friends_list, friends_limit))
        offset += 200
        friends_list = get_friends_list(user_id, offset)
    return counting_friends
            
def output(groups_list, friends_limit=0):
    output_groups = []
    with alive_bar(len(groups_list)) as bar:
        print("\nMatching friends and groups...")
        for group in groups_list:
            bar()
            if sum(match_groups_friends(group, friends_limit, user_id)) <= friends_limit:
...
def get_group_info(group_id):
    ...
    response = api_request(add_params)["response"]
    try:
        group_dict = {'name': response[0]['name'], 'id': response[0]['id'], 'members_count': response[0]['members_count']}
        return group_dict
    except KeyError:
        group_dict = {'name': response[0]['name'], 'id': response[0]['id'], 'members_count': 'N/A'}
        return group_dict
...
if __name__ == "__main__":
    while True:
        ...
        try:
            ...
            output_groups = output(groups_list, friends_limit=0)
            write_json(output_groups)

            print('\nDone! All information was saved to "groups.json" file.')
            break
        except TypeError:
            print("Process can't be managed")
            break
